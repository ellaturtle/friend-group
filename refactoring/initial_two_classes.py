...
class Group:
    ...
    def add_person(self, name, age, job):
        ...
        if self.contains(name):
            raise ValueError(f"A person named {name} is already in the group")
    ...
    def number_of_connections(self, name):
        ...
        if name not in self.connections:
            raise ValueError(f"No person named {name} in the group")
    ...
    def connect(self, name1, name2, relation, reciprocal=True):
        ...
        if not self.contains(name1) or not self.contains(name2):
            raise ValueError(f"Both members must exist in the group")
    ...
    def forget(self, name1, name2):
        ...
        if not self.contains(name1) or not self.contains(name2):
            raise ValueError(f"There is already no connection to this person.")
